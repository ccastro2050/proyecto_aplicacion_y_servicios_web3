...
- [ ] Crear el `docker-compose.yml` con los servicios `sqlserver` (imagen
      2022, volumen `mssqldata`, puerto 11465, healthcheck con sqlcmd) y
...
`sqlserver-init` en `Exited (0)`; un cliente SQL a `localhost:11465`
...
- [ ] `appsettings.json` con la cadena de conexión (default
      `localhost,11465` para correr sin Docker).
...
- [ ] `api_facturas/Dockerfile`: imagen `dotnet/sdk:10.0`, `dotnet watch`,
      `ASPNETCORE_URLS` en 8034, `DOTNET_USE_POLLING_FILE_WATCHER`.
- [ ] Agregar al `docker-compose.yml` el servicio `api-facturas`: `build:`,
      código montado + `bin/` y `obj/` en volúmenes anónimos, puerto 8034,
