...
└── api_facturas/
    ...
    ├── Dockerfile                    # sdk:10.0 + dotnet watch (puerto 8034)
    ├── Modelos/
...
funcionando. En v1 eso son **tres servicios**: `sqlserver` (11465 al host),
`sqlserver-init` (corre una vez) y `api-facturas` (8034, código montado +
