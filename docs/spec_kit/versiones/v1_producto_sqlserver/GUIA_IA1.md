...
8. En mi máquina TAMBIÉN corre el proyecto clonado del curso con sus
   ...
   a. Publica los puertos del host con +100: en el docker-compose.yml la
      API va "8134:8034" y SQL Server va "11565:1433" (adentro de los
   ...
   Docker) apunta a localhost,11565. Cuando me des URLs o comandos de
   prueba, usa localhost:8134 (API) y localhost,11565 (BD).
...
3. **El punto de control real es el smoke test final** (7_quickstart.md):
   ...
   (regla 8 del prompt) — donde el quickstart diga `localhost:8034` use
   `localhost:8134`, y donde diga `11465` use `11565`.
...
5. En esta máquina TAMBIÉN corre el proyecto clonado del curso. MI proyecto
   publica los puertos del host con +100 (API "8134:8034", SQL Server
   "11565:1433") y su docker-compose.yml empieza con `name: mi_v1_producto`.
6. Al final, corre el smoke test completo de 7_quickstart.md §2 (con mis
   puertos: localhost:8134) y muéstrame la evidencia de los 6 criterios de
