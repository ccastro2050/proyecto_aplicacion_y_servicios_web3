...
> provisto `db/bdfacturas.sql` + `db/init.sh` — no se genera ni se
...
persona ──< vendedor ─┼──< factura ──< productosporfactura >── producto
        (por id)      │      │              (el detalle)        (v1)
                      │      └ numero IDENTITY · fecha · total · estado
...
| `factura` | `numero` (IDENTITY), `fecha` (default GETDATE), `total` (lo fija el trigger), `estado` ('activa'/'anulada'), `fkidcliente`, `fkidvendedor`. Semilla: facturas 1–6 |
...
1. **Validar stock**: si `cantidad > stock` → `THROW` con el mensaje
