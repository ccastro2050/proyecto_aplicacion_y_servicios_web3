...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8034
EXPOSE 8034
...
| `EXPOSE 8034` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8034"]
...
```yaml
  sqlserver:
    ...
    ports:
      - "11465:1433"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8034`, `localhost,11465`); entre contenedores, nombres de
...
docker network create proyecto_aplicacion_y_servicios_web3_default

# 2. sqlserver
docker run -d --name sqlserver --network proyecto_aplicacion_y_servicios_web3_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11465:1433 mcr.microsoft.com/mssql/server:2022-latest

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_aplicacion_y_servicios_web3_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init.sh

# 5. Construir la imagen de api-facturas y encenderla
docker build -t api-facturas ./api_facturas
docker run -d --name api-facturas --network proyecto_aplicacion_y_servicios_web3_default --restart unless-stopped `
  ...
  -v /app/bin `
  -v /app/obj `
  -p 8034:8034 api-facturas

```

**5 comandos**, con sus flags, en un orden que no se puede equivocar.
...
```yaml
URL_API: http://api-facturas:8034
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8034` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8034` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8034:8034`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8034`, el front
...
```powershell
docker network inspect proyecto_aplicacion_y_servicios_web3_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_aplicacion_y_servicios_web3` produce
la red `proyecto_aplicacion_y_servicios_web3_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_aplicacion_y_servicios_web3-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_aplicacion_y_servicios_web3_default has active endpoints (name:"proyecto_aplicacion_y_servicios_web3-api-facturas-1" …)
```
...
docker compose exec api-facturas getent hosts sqlserver
