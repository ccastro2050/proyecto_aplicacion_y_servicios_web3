...
**publicado** en el puerto **11465** de su PC. En el diálogo **Conectar**
(pestaña *Connection Properties*) llene así:

| Campo | Valor |
|---|---|
| **Server Name** | `localhost,11465` |
...
- El puerto va con **COMA**, no con dos puntos: `localhost,11465`. Es la
...
- **Resultados** (abajo): los **8 productos de fábrica** (PR001–PR008)
  ...
  `http://localhost:8034/api/producto`: es la MISMA información — una
  vista por SQL y otra por la API.
- **La barra de estado** (abajo a la derecha) resume la sesión completa:
  servidor `localhost,11465`, usuario `sa`, la BD y **8 filas**.
...
- Véalo también por la otra puerta: `http://localhost:8034/api/producto`
